...
export class HyseimDebugger extends DebugSession {
	...
	public constructor(debuggerLinesStartAt1: boolean, isServer: boolean = false) {
		...
		process.on('unhandledRejection', (reason, p) => {
			p.catch((e) => {
				...
				this.debugConsole.errorUser('[hyseim debug] Unhandled Rejection: ' + errorMessage(e));
			});
		});
		...
		process.on('uncaughtException', (err) => {
			...
			this.debugConsole.errorUser('[hyseim debug] Fatal error during debugging session. Catched unhandled exception.\n' + errorStack(err));
			setTimeout(() => {
				process.exit(1);
			}, 2000);
		});
