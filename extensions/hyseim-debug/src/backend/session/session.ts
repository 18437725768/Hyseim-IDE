...
export class DebuggingSession {
	...
	constructor(
		...
		/* COMMAND SERVER */
		const tempPath = systemPath.join(tmpdir(), 'hyseim-debug-sockets');
		...
		commandServer.on('error', err => {
			this.logger.error('Hyseim-Debug WARNING: Utility Command Server: Error in command socket ' + err.toString());
		});
	...
	public async connect(load: boolean) {
		...
		this.debugConsole.logUser('[hyseim debug] debugger starting...');
		this.logger.info(`[hyseim debug] debugger starting: test log.`);
	...
	public async terminate() {
		this.debugConsole.logUser('[hyseim debug] debugger stopping.');
		await this.dispose();
