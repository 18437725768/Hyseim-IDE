...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(vscode.commands.registerCommand('hyseim-debug.examineMemoryLocation', examineMemory));
	context.subscriptions.push(vscode.commands.registerCommand('hyseim-debug.runWithoutDebug', (arg: any) => runWithoutDebug(arg)));
	context.subscriptions.push(vscode.commands.registerCommand('hyseim-debug.getFileNameNoExt', () => {
	...
	context.subscriptions.push(vscode.commands.registerCommand('hyseim-debug.getFileBasenameNoExt', () => {
		if (!vscode.window.activeTextEditor || !vscode.window.activeTextEditor.document || !vscode.window.activeTextEditor.document.fileName) {
			vscode.window.showErrorMessage('No editor with valid file name active');
			return;
		}
	...
	vscode.debug.registerDebugConfigurationProvider('hyseim', new Provider());
...
function examineMemory() {
	const socketlists = path.join(os.tmpdir(), 'hyseim-debug-sockets');
...
class MemoryContentProvider implements vscode.TextDocumentContentProvider {
	provideTextDocumentContent(uri: vscode.Uri, token: vscode.CancellationToken): Thenable<string> {
		return new Promise((resolve, reject) => {
			const conn = net.connect(path.join(os.tmpdir(), 'hyseim-debug-sockets', uri.authority));
			let from, to;
			let highlightAt = -1;
