...
const configOverwrites: { [id: string]: SettingsOverwiter<any> } = {
	...
	'C_Cpp.default.compilerPath'(access: ServicesAccessor) {
		const nodePathService = access.get<INodePathService>(INodePathService);
		return resolvePath(nodePathService.getToolchainBinPath(), 'riscv32-unknown-elf-gcc' + executableExtension);
	},
	'C_Cpp.default.includePath'(access: ServicesAccessor) {
		...
		ret.push(resolvePath(toolchain, 'riscv32-unknown-elf/include'));

		const libgcc = resolvePath(toolchain, 'lib/gcc/riscv32-unknown-elf');
		...
		const libcpp = resolvePath(toolchain, 'riscv32-unknown-elf/include/c++');
		const libcppVersion = readdirSync(libcpp)[0];
		ret.push(resolvePath(libcpp, libcppVersion));
		ret.push(resolvePath(libcpp, libcppVersion, 'riscv32-unknown-elf'));
...
setIfNot.set('search.location', 'sideber');
