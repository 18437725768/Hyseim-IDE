...
import { IHyseimStatusControllerService, IPublicStatusButton, StatusBarLeftLocation } from 'vs/hyseim/vs/workbench/bottomBar/common/type';
import {
...
import { CMakeStatus, CONTEXT_CMAKE_STATUS, ICMakeService } from 'vs/hyseim/vs/workbench/cmake/common/type';
import { ACTION_ID_SERIAL_MONITOR_TOGGLE, ACTION_LABEL_SERIAL_MONITOR_TOGGLE } from 'vs/hyseim/vs/workbench/serialMonitor/common/actionId';
...
import { CONTEXT_KENDRYTE_MULTIPLE_PROJECT } from 'vs/hyseim/vs/services/workspace/common/contextKey';
...
import { ACTION_ID_MAIX_CMAKE_HELLO_WORLD } from 'vs/hyseim/vs/workbench/cmake/common/actionIds';
...
class HyseimButtonContribution extends Disposable implements IWorkbenchContribution {
	private currentHasError: boolean = false;

	constructor(
		@IHyseimStatusControllerService private readonly statusControl: IHyseimStatusControllerService,
	...
	private createCMakeButtons() {
		...
		this._register(this.hyseimWorkspaceService.onCurrentWorkingDirectoryChange((path) => {
			this.onWorkspaceChange(workspaceButton, path);
		}));
		this.onWorkspaceChange(workspaceButton, this.hyseimWorkspaceService.getCurrentWorkspace());
					...
					switch (error.type) {
						case CMakeErrorType.NO_WORKSPACE:
							if (this.hyseimWorkspaceService.isEmpty()) {
								errorButton.command = this.hyseimWorkspaceService.isEmptyWorkspace() ? 'workbench.action.addRootFolder' : 'workbench.action.files.openFolder';
	...
	private onWorkspaceChange(workspaceButton: IPublicStatusButton, path: void | string) {
		...
		this.hyseimWorkspaceService.readProjectSetting(path).then((data) => {
			if (data) {
...
workbenchRegistry.registerWorkbenchContribution(HyseimButtonContribution, LifecyclePhase.Ready);
