...
import { PackageBrowserInput } from 'vs/hyseim/vs/workbench/packageManager/common/editors/packageBrowserInput';
import { IRemotePackageInfo, PACKAGE_LIST_EXAMPLE, PACKAGE_LIST_LIBRARY } from 'vs/hyseim/vs/workbench/packageManager/common/distribute';
...
import { parseExtendedJson } from 'vs/hyseim/vs/base/common/jsonComments';
import { IChannelLogService } from 'vs/hyseim/vs/services/channelLogger/common/type';
import { ILogService } from 'vs/platform/log/common/log';
import { IFileCompressService } from 'vs/hyseim/vs/services/fileCompress/node/fileCompressService';
import { CMAKE_CONFIG_FILE_NAME, CMAKE_LIBRARY_FOLDER_NAME } from 'vs/hyseim/vs/base/common/constants/wellknownFiles';
import { CMakeProjectTypes, ICompileInfo, ILibraryProject } from 'vs/hyseim/vs/base/common/jsonSchemas/cmakeConfigSchema';
...
import { dumpDate } from 'vs/hyseim/vs/base/common/dumpDate';
import { unClosableNotify } from 'vs/hyseim/vs/workbench/progress/common/unClosableNotify';
import { INotificationHandle, INotificationService, Severity } from 'vs/platform/notification/common/notification';
import { PACKAGE_MANAGER_DISTRIBUTE_URL } from 'vs/hyseim/vs/base/common/constants/remoteRegistry';
import { INodeDownloadService } from 'vs/hyseim/vs/services/download/common/download';
...
import { packageJsonObject, packageJsonString } from 'vs/hyseim/vs/base/common/cmakeTypeHelper';
...
export class PackageRegistryService implements IPackageRegistryService {
	...
	public async listLocal(projectPath?: string): Promise<ILibraryProject[]> {
		if (!projectPath) {
			projectPath = this.hyseimWorkspaceService.getCurrentWorkspace();
			if (!projectPath) {
				return [];
			}
		}
		...
		for (const item of await readDirsInDir(folder)) {
			const result = await this.hyseimWorkspaceService.readProjectSetting(resolvePath(folder, item)) as ILibraryProject;
			if (!result) {
				continue;
			}
	...
	openPackageFile(projectPath: string, sideByside: boolean = false): Promise<any> {
		return this.editorService.openEditor({
			resource: URI.file(this.hyseimWorkspaceService.getProjectSetting(projectPath)),
		});
	}
	...
	public async installAll(): Promise<void> {
		const workspaces = await this.hyseimWorkspaceService.getAllWorkspace();
	...
	public installProject(dir = this.hyseimWorkspaceService.getCurrentWorkspace()) {
		if (!dir) {
	...
	public async _installSingleWorkspace(workspacePath: string, handle: INotificationHandle): Promise<void> {
		const pkgInfo = await this.hyseimWorkspaceService.readProjectSetting(workspacePath);
		if (!pkgInfo) {
	...
	public async installDependency(packageInfo: IRemotePackageInfo, version?: string): Promise<void> {
		...
		const currentConfigFile = this.hyseimWorkspaceService.getProjectSetting(
			this.hyseimWorkspaceService.requireCurrentWorkspace(),
		);
	...
	private async downloadFromAbsUrl(downloadUrl: string, defaultName: string, defaultVersion: string | undefined): Promise<string> {
		...
		const packagesRoot = this.hyseimWorkspaceService.requireCurrentWorkspaceFile(CMAKE_LIBRARY_FOLDER_NAME);
	...
	public async erasePackage(packageName: string) {
		const activeProject = this.hyseimWorkspaceService.requireCurrentWorkspace();

		const packageSaveDir = resolvePath(activeProject, CMAKE_LIBRARY_FOLDER_NAME, packageName);
		const currentConfigFile = this.hyseimWorkspaceService.getProjectSetting(activeProject);
