...
export class MaixCMakeRunAction extends Action {
	...
	async run() {
		...
		const gdb = resolvePath(this.nodePathService.getToolchainBinPath(), 'riscv32-unknown-elf-gdb' + executableExtension);
		return this.commandService.executeCommand('hyseim-debug.runWithoutDebug', {
			app,
			gdb,
