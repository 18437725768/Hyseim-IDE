import {
	...
	ICMakeService,
	ICMakeStatus,
} from 'vs/hyseim/vs/workbench/cmake/common/type';
...
import {
	...
	ICMakeRequest,
	ICMakeResponse,
} from 'vs/hyseim/vs/workbench/cmake/common/cmakeProtocol/cmakeProtocol';
import { INotificationService } from 'vs/platform/notification/common/notification';
import { unlinkSync } from 'fs';
import { Emitter, Event } from 'vs/base/common/event';
import { ICMakeProtocolError } from 'vs/hyseim/vs/workbench/cmake/common/cmakeProtocol/error';
import { ICMakeProtocolMessage, ICMakeProtocolProgress, ICMakeProtocolReply } from 'vs/hyseim/vs/workbench/cmake/common/cmakeProtocol/event';
import { ICMakeProtocolSignal } from 'vs/hyseim/vs/workbench/cmake/common/cmakeProtocol/singal';
import {
...
} from 'vs/hyseim/vs/workbench/cmake/common/cmakeProtocol/message';
import { CMakeCache } from 'vs/hyseim/vs/workbench/cmake/node/cmakeCache';
...
import { DebugScript, getLimitedEnvironment } from 'vs/hyseim/vs/workbench/cmake/node/environmentVars';
import { executableExtension, PathListSep, removeEnvironment } from 'vs/hyseim/vs/base/common/platformEnv';
import { CMakeBuildErrorProcessor, CMakeBuildProgressProcessor, CMakeProcessList } from 'vs/hyseim/vs/workbench/cmake/node/outputProcessor';
import { IConfigurationService } from 'vs/platform/configuration/common/configuration';
import { ICommandService } from 'vs/platform/commands/common/commands';
import { CMAKE_LIST_GENERATED_WARNING, CMAKE_LIST_GENERATED_WARNING_OLD } from 'vs/hyseim/vs/base/common/jsonSchemas/cmakeConfigSchema';
...
import { CONFIG_KEY_CMAKE_DEBUG, CONFIG_KEY_MAKE_PROGRAM } from 'vs/hyseim/vs/base/common/configKeys';
import { IHyseimStatusControllerService } from 'vs/hyseim/vs/workbench/bottomBar/common/type';
import { IContextKeyService } from 'vs/platform/contextkey/common/contextkey';
import { CMakeError, CMakeErrorType } from 'vs/hyseim/vs/workbench/cmake/common/errors';
import { DeferredPromise } from 'vs/hyseim/vs/base/common/deferredPromise';
import { localize } from 'vs/nls';
import { CMAKE_CONFIG_FILE_NAME, PROJECT_BUILD_FOLDER_NAME } from 'vs/hyseim/vs/base/common/constants/wellknownFiles';
...
import { createSimpleErrorMarker, createSimpleMarker } from 'vs/hyseim/vs/platform/marker/common/simple';
import { MapLike } from 'vs/hyseim/vs/base/common/extendMap';
import { filterProjectName } from 'vs/hyseim/vs/base/common/filterProjectName';
...
export class CMakeService implements ICMakeService {
	...
	constructor(
		...
		@IHyseimStatusControllerService private readonly hyseimStatusControllerService: IHyseimStatusControllerService,
		...
		this.hyseimWorkspaceService.onCurrentWorkingDirectoryChange(() => {
	...
	private async getCMakeDef(): Promise<{ [name: string]: string }> {
		const currentFolder = this.hyseimWorkspaceService.requireCurrentWorkspace();
	...
	protected async runCMakeRaw(...args: string[]) {
		...
		await dbg.writeBack(this.hyseimWorkspaceService.requireCurrentWorkspace(), 'last-cmake-command');
	...
	private async ensureProcess() {
		...
		const dbg = new DebugScript(options.cwd, options.env);
		dbg.command(cmakePath.cmake, args);
		dbg.writeBack(this.hyseimWorkspaceService.requireCurrentWorkspace(), 'cmake-server');
		...
		switch (protocolData.type) {
			...
			case CMAKE_EVENT_TYPE.MESSAGE:
				...
				if (regCMakeConfigureError.test(message.message)) {
					...
					const packageFile = URI.file(this.hyseimWorkspaceService.requireCurrentWorkspaceFile(file.replace('CMakeLists.txt', CMAKE_CONFIG_FILE_NAME)));
					...
					this.markerService.changeOne(
						CMAKE_ERROR_MARKER,
						URI.file(this.hyseimWorkspaceService.requireCurrentWorkspaceFile(file)),
						[marker],
					);
	...
	private async _rescanCurrentFolder(currentFolder: string | void): Promise<void> {
		...
		const projectName = await this.hyseimWorkspaceService.getCurrentProjectName();
	...
	private get cmakeLists() {
		if (this._CMakeProjectExists) {
			return this.hyseimWorkspaceService.requireCurrentWorkspaceFile('CMakeLists.txt');
		} else {
			const currentFolder = this.hyseimWorkspaceService.getCurrentWorkspace();
	...
	private async _build() {
		...
		const processors = this.lastProcess = new CMakeProcessList([
			new CMakeBuildProgressProcessor(this.hyseimStatusControllerService),
			errorProcessor,
		]);
		...
		dbg.writeBack(this.hyseimWorkspaceService.requireCurrentWorkspace(), 'cmake-build');
	...
	private async notifyCppExtension() {
		...
		const to = this.hyseimWorkspaceService.requireCurrentWorkspaceFile('.vscode/compile_commands.backup.json');
		content.configurations.unshift({
			name: 'Default',
			defines: [],
			compilerPath: resolvePath(this.nodePathService.getToolchainBinPath(), 'riscv32-unknown-elf-g++'),
			cStandard: 'c11',
			cppStandard: 'c++17',
		...
		const from = this.hyseimWorkspaceService.requireCurrentWorkspaceFile('build/compile_commands.json');
