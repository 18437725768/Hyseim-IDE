...
import { ACTION_ID_CREATE_SHORTCUTS, ACTION_LABEL_CREATE_SHORTCUTS } from 'vs/hyseim/vs/base/common/menu/tools';
import { isLinux, isWindows } from 'vs/base/common/platform';
import { Extensions as WorkbenchExtensions, IWorkbenchContribution, IWorkbenchContributionsRegistry } from 'vs/workbench/common/contributions';
import { LifecyclePhase } from 'vs/platform/lifecycle/common/lifecycle';
import { Disposable } from 'vs/base/common/lifecycle';
import { IStorageService, StorageScope } from 'vs/platform/storage/common/storage';
import { promiseWithNotificationService } from 'vs/hyseim/vs/platform/notify/common/promiseNotification';
...
Registry.as<IWorkbenchActionRegistry>(ActionExtensions.WorkbenchActions)
	.registerWorkbenchAction(new SyncActionDescriptor(CreateShortcutsAction, CreateShortcutsAction.ID, CreateShortcutsAction.LABEL), 'Hyseim: Create shortcuts', category);
...
class FirstBootContribution extends Disposable implements IWorkbenchContribution {
	private readonly storageKey = 'hyseim.firstrun';
	...
	private message() {
		if (isWindows) {
			return localize('confirmCreateLink', 'Create a start menu shortcut for Hyseim IDE?');
		} else if (isLinux) {
			return localize('confirmCreateLink', 'Create a system menu item for Hyseim IDE?');
		} else {
			return localize('confirmCreateLink', 'Create a launchpad icon for Hyseim IDE?');
		}
	}
