import { DefineValue, IProjectInfo } from 'vs/hyseim/vs/services/makefileService/common/type';
import { missingJsonField, missingOrInvalidProject } from 'vs/hyseim/vs/base/common/messages';
import { CMakeProjectTypes, ICompileFolder, ILibraryProject } from 'vs/hyseim/vs/base/common/jsonSchemas/cmakeConfigSchema';
import { relativePath, resolvePath } from 'vs/hyseim/vs/base/common/resolvePath';
import { CMAKE_LIBRARY_FOLDER_NAME } from 'vs/hyseim/vs/base/common/constants/wellknownFiles';
import { ExtendMap } from 'vs/hyseim/vs/base/common/extendMap';
import { ignorePattern } from 'vs/hyseim/vs/platform/fileDialog/common/globalIgnore';
...
import { isSameDrive } from 'vs/hyseim/vs/base/common/fs/isSameDrive';
import { PathAttachedError } from 'vs/hyseim/vs/platform/marker/common/errorWithPath';
import { exists } from 'vs/base/node/pfs';
import { basename } from 'vs/base/common/path';
import { DeepReadonlyArray } from 'vs/hyseim/vs/base/common/type/deepReadonly';
import { arrayRemoveDuplicate } from 'vs/hyseim/vs/base/common/arrayUnique';
import { filterProjectName } from 'vs/hyseim/vs/base/common/filterProjectName';
import { packageJsonArray, packageJsonIsNormal, packageJsonObject } from 'vs/hyseim/vs/base/common/cmakeTypeHelper';
...
export class MakefileServiceResolve {
	...
	private async _readProjectJsonList(projectPath: string): Promise<IProjectInfo> {
		...
		const projectJson = await this.hyseimWorkspaceService.readProjectSetting(projectPath);
		if (!projectJson) {
		...
		const handleErr = (e: Error) => {
			...
			const ee = new PathAttachedError(this.hyseimWorkspaceService.getProjectSetting(projectPath), e.message);
			ee.stack = e.stack;
			throw ee;
		};
		...
		if (localDependency && localDependency.length) {
			for (const relPath of localDependency) {
				...
				if (await exists(this.hyseimWorkspaceService.getProjectSetting(absolutePath))) {
